...
@Component
public class ProdutoJpaAdapterPort implements ProdutoRepositoryPort {
    
    private final ProdutoRepositoryImpl repository;
    private final ProdutoMapper mapper;
    
    public ProdutoJpaAdapterPort(ProdutoRepositoryImpl repository, ProdutoMapper mapper) {
    ...
    @Override
    public Page<Produto> listarPorMarca(UUID idMarca, int pagina, int tamanho) {
        PageRequest pageRequest = PageRequest.of(pagina, tamanho);
        return repository.findByIdMarca(idMarca, pageRequest)
                .map(mapper::toDomain);
    }
